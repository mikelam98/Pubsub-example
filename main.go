...
func pricePublisher(broker *pubsub.Broker) {
	topicsKeys := make([]string, 0, len(availableTopics))
	topicsValues := make([]string, 0, len(availableTopics))
	for k, v := range availableTopics {
		topicsKeys = append(topicsKeys, k)
		topicsValues = append(topicsValues, v)
	}
	for {
		randValues := topicsValues[rand.Intn(len(topicsValues))]
		msg := fmt.Sprintf("%f", rand.Float64())
		//fmt.Printf("Publishing %s to %s topic\n",msg,randKeys)
		go broker.Publish(randValues, msg)
		go broker.Broadcast(msg, topicsValues)
		r := rand.Intn(4)
...
func main() {
	...
	broker := pubsub.NewBroker() // create new subscriber
